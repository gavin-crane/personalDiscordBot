...
class MusicCog(commands.Cog):
    ...
    async def play(self, ctx, *, query: str):
        ...
        global discord_player
        discord_player = self.bot.lavalink.player_manager.get(ctx.guild.id)
        ...
        results = await discord_player.node.get_tracks(query)
        ...
        if results.load_type == 'PLAYLIST_LOADED':
            ...
            for track in tracks:
                ...
                discord_player.add(requester=ctx.author.id, track=track)
        ...
        # the current track.
        if not discord_player.is_playing:
            await discord_player.play()
    ...
    async def skip(self, ctx):
      ...
      if not ctx.author.voice or (discord_player.is_connected and ctx.author.voice.channel.id != int(discord_player.channel_id)):
            # Abuse prevention. Users not in voice channels, or not in the same voice channel as the bot
            # may not disconnect the bot.
            return await ctx.send('You\'re not in my voicechannel!')
